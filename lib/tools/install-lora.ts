...
export const installLoraToolName = "install_lora_from_url";

const installLoraInputSchema = z.object({
  url: z.string().url("A valid download URL is required."),
  baseModel: z
    .string()
    .min(1)
    .transform((value) => value.trim().toLowerCase())
    .pipe(z.enum(["sdxl", "chroma", "qwen"])),
  fileName: z.string().min(1).optional(),
  overwrite: z.boolean().optional(),
});

const baseModelToSubfolder: Record<"sdxl" | "chroma" | "qwen", string> = {
  sdxl: "illustration",
  chroma: "chroma",
  qwen: "qwen",
};
...
export const registerInstallLoraMcpTool = (server: McpServer) => {
  server.registerTool(
    installLoraToolName,
    {
      title: "Install LoRA From URL",
      description:
        "Downloads a LoRA file from URL and installs it into Comfy LoRA subfolder by base model (sdxl -> illustration, chroma -> chroma, qwen -> qwen).",
      inputSchema: installLoraInputSchema,
    },
    async (input) => {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Download failed (${response.status}). ${details.slice(0, 500)}`,
          );
        }
          ...
          content: [
            {
              ...
              text:
                error instanceof Error
                  ? error.message
                  : "Failed to install LoRA from URL.",
            },
          ],
          isError: true,
        };
