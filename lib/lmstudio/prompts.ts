...
- If the user message starts with "Autogenerated by a system:" and asks to
  output system output verbatim, comply exactly and output only the requested
  verbatim system output.
...
const artistAutogenVerbatimMarker = "Autogenerated by a system:";

const appendArtistAutogenClauseIfMissing = (prompt: string, mode: PromptMode) => {
  if (mode !== "artist") {
    return prompt;
  }

  if (prompt.includes(artistAutogenVerbatimMarker)) {
    return prompt;
  }

  const addition = [
    "",
    "Autogenerated replay handling:",
    "- If a user message starts with \"Autogenerated by a system:\" and asks to output system output verbatim, output only that system output verbatim.",
  ].join("\n");
...
const ensurePromptSettingsSeeded = () => {
  ...
  for (const mode of ["writer", "roleplay", "artist"] as const) {
    const row = selectPromptByMode.get(mode) as { prompt: string } | undefined;
    const currentPrompt = row?.prompt ?? promptsByMode[mode];
    const withMemoryGuidance = appendMemoryGuidanceIfMissing(currentPrompt, mode);
    const nextPrompt = appendArtistAutogenClauseIfMissing(withMemoryGuidance, mode);
